-- espnew_fixed.lua
-- [FIX #3] Two targeted patches applied to this library:
--
--   PATCH A — tool_added now gates on flags["Weapon"] before reparenting the
--              weapon label to the visible holder.  Previously it showed the
--              weapon ESP unconditionally whenever a tool was present, causing
--              the label to appear on respawn even when the toggle was OFF.
--
--   PATCH B — refresh_descendants defers a call to esp.refresh_elements()
--              after the new character is fully wired up.  This guarantees
--              every Drawing/GUI object is snapped back to the current flag
--              states the moment a player respawns, with no toggle cycling
...
-- Reusable RaycastParams for Wall Check (Massive performance saver)
...
local flags = {
    ["Enabled"] = false;
    ["Names"] = false;["Name_Color"] = { Color = rgb(0, 255, 255) };["Boxes"] = false;
    ["Box_Type"] = "Corner";
    ["Box_Color"] = { Color = rgb(0, 255, 0) };
    ["Healthbar"] = false;["Health_High"] = { Color = rgb(0, 255, 0) };["Health_Low"] = { Color = rgb(255, 0, 0) };["Distance"] = false;
    ["Weapon"] = false;
    ["Skeletons"] = false;
    ...
    -- Functions 
        ...
        function esp:create_object( player )
            ...
            local objects = data.objects; do
                ...
                -- Weapon esp
                    objects[ "weapon" ] = esp:create( "TextLabel" , {
                        ...
                        BorderColor3 = rgb(0, 0, 0);
                        Text = "[ak-47]";
                        -- [FIX #3 PATCH A] Always start in the cache; tool_added
                        -- will move it to the holder only when flags["Weapon"] is ON.
                        Parent = esp.cache;
                        TextStrokeTransparency = 0;
                        Name = "\0";
                        Size = dim2(1, 0, 0, 0);
            ...
            do --[[ data functions ]]
                ...
                -- [FIX #3 PATCH A] tool_added now checks flags["Weapon"] before
                -- deciding whether to show the weapon label.  Previously it would
                -- unconditionally reparent the label to the visible holder whenever
                -- a tool existed, bypassing the toggle entirely on respawn.
                data.tool_added = function( item )
                    ...
                    -- Respect the Weapon ESP flag: only show if the toggle AND a
                    -- tool are both present; otherwise send to the invisible cache.
                    objects[ "weapon" ].Parent = (flags["Weapon"] and exists)
                        and objects[ "holder" ]
                        or  esp.cache
                end
                ...
                -- [FIX #3 PATCH B] After wiring up the new character, defer a full
                -- refresh_elements() pass.  This snaps every ESP object (boxes,
                -- names, weapon, health, skeletons, etc.) to its current flag state
                -- the moment the respawned character is ready — no toggle cycling.
                data.refresh_descendants = function() 
                    ...
                    -- Re-sync all ESP visuals to the current toggle states on every
                    -- respawn.  task.defer yields until after this frame so all
                    -- character children are settled before we read them.
                    task.defer(esp.refresh_elements)
                end
            end 
        ...
        function esp.refresh_elements( )
            for _,v in players:GetPlayers() do 
                ...
                -- [FIX #3 PATCH A continued] refresh_elements also respects
                -- flags["Weapon"] — it always sets weapon visibility correctly,
                -- which covers the case where the master toggle cycles.
                objects[ "weapon" ].Parent = flags["Weapon"]
                    and v.Character:FindFirstChildOfClass("Tool")
                    and objects[ "holder" ]
                    or  esp.cache
        ...
        esp.connection = run.RenderStepped:Connect(function()
            ...
            for _, player in players:GetPlayers() do 
                ...
                -- ==== 1. TEAM & INVISIBLE CHECK ====
                local is_teammate = flags["TeamCheck"] and player.Team ~= nil and player.Team == players.LocalPlayer.Team
                local is_invisible = false

                if flags["InvisibleCheck"] then
                ...
                if is_teammate or is_invisible then
                    -- Hide main UI holder
                    if objects["holder"].Visible then
                ...
                -- ==== 2. WALL CHECK ====
                if flags["WallCheck"] and on_screen and humanoid.RootPart then
                    local localChar = players.LocalPlayer.Character
                    raycast_params.FilterDescendantsInstances = localChar and {camera, localChar} or {camera}

                    local origin = camera.CFrame.Position
                    local direction = humanoid.RootPart.Position - origin
                    
                    local hit = workspace:Raycast(origin, direction, raycast_params)
                    ...
                    if flags["Skeletons"] and character:FindFirstChild("UpperTorso") then 
                        for i = 1, #bones do
                            ...
                            if origin_3d and destination_3d then 
                                ...
                                if on_screen and on_screen_start and on_screen_end then 
                                    path.Visible = true
...
flags = { 
    ["Enabled"] = false;
    ["Names"] = false;
    ["Name_Color"] = { Color = rgb(255, 255, 255) };["Boxes"] = false;
    ["Box_Type"] = "Normal";
    ["Box_Color"] = { Color = rgb(255, 255, 255) };["Healthbar"] = false; 
    ["Health_High"] = { Color = rgb(0, 255, 0) };
    ["Health_Low"] = { Color = rgb(255, 0, 0) };
    ["Distance"] = false;
    ["Weapon"] = false;["Skeletons"] = false;
    ["Skeletons_Color"] = { Color = rgb(255, 255, 255) };
    ["Distance_Color"] = { Color = rgb(255, 255, 255) };
    ["Weapon_Color"] = { Color = rgb(255, 255, 255) };["InvisibleCheck"] = false;
    ["WallCheck"] = false; 
    ["TeamCheck"] = false; 
}; 
task.wait()
