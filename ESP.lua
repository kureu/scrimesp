...
local UtilityFunctions = {
    InitChecks = function(self, Entry)
		...
		--// Cached RaycastParams for Wall Check
		...
		Entry.Connections.UpdateChecks = Connect(__index(RunService, DeveloperSettings.UpdateMode), function()
			...
			local LocalCharacterPosition = CoreFunctions.GetLocalCharacterPosition() -- Update local position dynamically
			...
			if Character and IsDescendantOf(Character, Workspace) and Humanoid and Head then -- Player
				...
				--// Ghost Check
				if Settings.GhostCheck and __index(Head, "Transparency") >= 0.95 then
					IsGhost = true
				end
				...
				--// Wall Check
				if Settings.WallCheck and Checks.Alive and Checks.Team and IsInDistance and not IsGhost then
					-- Filter out the LocalPlayer's character and the Camera
					CastParams.FilterDescendantsInstances = {LocalPlayer.Character, CurrentCamera}
					
					local Origin = __index(CurrentCamera, "CFrame").Position
					local TargetPos = __index(Head, "Position")
					local Direction = TargetPos - Origin
					
					local Result = Workspace:Raycast(Workspace, Origin, Direction, CastParams)

					-- If ray hits something, and that something is NOT part of the target character, then it's a wall.
					if Result and not IsDescendantOf(Result.Instance, Character) then
						IsVisible = false
					end
				end

				Checks.Ready = Checks.Alive and Checks.Team and not Settings.PartsOnly and IsInDistance and not IsGhost and IsVisible
			else
				Checks.Alive = false
				Checks.Team = false
				Checks.Ready = false
